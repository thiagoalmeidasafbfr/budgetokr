              ...
              {FIELDS.map(f => (
                <div key={f.key}>
                  ...
                  <input value={form[f.key] ?? ''} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}
                    ...
                    className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 disabled:bg-gray-50 disabled:text-gray-400" />
                </div>
              ))}
            </div>
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && <tr><td colSpan={7} className="text-center py-8 text-gray-400"><div className="w-5 h-5 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin mx-auto" /></td></tr>}
                ...
                <tr key={row.centro_custo} className="border-b border-gray-50 hover:bg-gray-50 group transition-colors">
                  ...
                  <td className="px-2 py-2 text-right">
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => startEdit(row)} className="p-1 text-gray-400 hover:text-indigo-500 transition-colors"><Edit2 size={12} /></button>
              ...
              {!loading && rows.length === 0 && <tr><td colSpan={7} className="text-center py-8 text-gray-400 text-sm">Nenhum registro. Importe via <a href="/upload" className="text-indigo-500 underline">Upload</a> ou adicione manualmente.</td></tr>}
            </tbody>
          </table>
        </div>
      </Card>
