      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && <tr><td colSpan={5} className="text-center py-8"><div className="w-5 h-5 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin mx-auto" /></td></tr>}
                ...
                <tr key={row.numero_conta_contabil} className="border-b border-gray-50 hover:bg-gray-50 group transition-colors">
                  ...
                  <td className="px-2 py-2 text-right">
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditing(row.numero_conta_contabil); setForm({ ...row }) }} className="p-1 text-gray-400 hover:text-indigo-500"><Edit2 size={12} /></button>
                      <button onClick={() => remove(row.numero_conta_contabil)} className="p-1 text-gray-400 hover:text-red-400"><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {!loading && rows.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-gray-400 text-sm">Nenhum registro. <a href="/upload" className="text-indigo-500 underline">Importe via Upload</a>.</td></tr>}
            </tbody>
          </table>
        </div>
      </Card>
