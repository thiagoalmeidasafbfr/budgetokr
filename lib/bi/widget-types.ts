...
export type BiPeriodo =
  ...
  | { tipo: 'lista'; periodos: string[] }         // arbitrary multi-month list
...
export type BiMetrica =
  ...
  | { tipo: 'medida';         medida_id: number; nome_medida: string }
...
export type BiQueryResult =
  | { tipo: 'escalar';   valor: number; comparativo: number | null; variacao_pct: number | null; label_valor?: string }
  | { tipo: 'serie';     pontos: Array<{ periodo: string; realizado: number; budget: number | null }> }
  | { tipo: 'breakdown'; itens: Array<{ label: string; realizado: number; budget: number | null; desvio_pct: number | null; participacao_pct: number }> }
  | { tipo: 'dre';       linhas: DreLine[] }
  | { tipo: 'topN';      itens: Array<{ label: string; valor: number }>; n: number; ordem: 'asc' | 'desc' }
...
export const WIDGET_META: WidgetMeta[] = [
  { visual: 'kpi_card',       label: 'KPI Card',          descricao: 'Valor único em destaque',      icon: '🎯', defaultW: 3,  defaultH: 2, defaultMetrica: { tipo: 'linha_dre', linha_nome: '' } },
  ...
  { visual: 'bar_vertical',   label: 'Barras Verticais',   descricao: 'Comparativo por período/dim.', icon: '📊', defaultW: 6,  defaultH: 4, defaultMetrica: { tipo: 'breakdown_dpto', linha_nome: '' } },
  { visual: 'bar_horizontal', label: 'Barras Horizontais', descricao: 'Ranking de entidades',         icon: '📶', defaultW: 6,  defaultH: 4, defaultMetrica: { tipo: 'topN_grupo', grupo_nome: '', n: 10, ordem: 'desc' } },
  { visual: 'line_area',      label: 'Linha / Área',       descricao: 'Evolução no tempo',            icon: '📈', defaultW: 8,  defaultH: 4, defaultMetrica: { tipo: 'serie_temporal', linha_nome: '' } },
  { visual: 'donut',          label: 'Rosca',              descricao: 'Distribuição percentual',      icon: '🍩', defaultW: 4,  defaultH: 4, defaultMetrica: { tipo: 'breakdown_dpto', linha_nome: '' } },
  { visual: 'pie',            label: 'Pizza',              descricao: 'Participação por fatia',       icon: '🥧', defaultW: 4,  defaultH: 4, defaultMetrica: { tipo: 'breakdown_cc', linha_nome: '' } },
