...
const DEPT_ALLOWED_PATHS = ['/dept', '/dre', '/dre-gerencial', '/analise', '/capex', '/unidades-negocio', '/api/dept-dashboard', '/api/dept-medidas', '/api/analise', '/api/dre', '/api/capex', '/api/kpis', '/api/kpis/valores', '/api/lancamentos', '/api/medidas', '/api/me', '/api/auth/logout', '/api/dimensoes', '/api/favorites', '/api/unidades-negocio', '/api/exec-chart', '/api/exec-chart-config']
...
export async function middleware(request: NextRequest) {
  ...
  if (user.role === 'dept') {
    // APIs onde dept pode escrever (comentários, favoritos e gráficos executivos)
    const DEPT_WRITE_ALLOWED = ['/api/dre/comments', '/api/favorites', '/api/exec-chart-config']
  ...
  const allDepts = user.departments ?? (user.department ? [user.department] : [])
  requestHeaders.set('x-user-depts', encodeURIComponent(allDepts.join(',')))
